...
def scrape_matches(url: str, cache_file: str, ipfs_name: str, force_refresh=False):
    ...
    2. Pinata IPFS (if local cache missing)
    3. Web scraping (if both fail or force_refresh)
    ...
    for attempt in range(1, 6):
        try:
            ...
            if response.status_code == 200:
                ...
                wait = 5 * attempt  # Longer wait for 403
    ...
    if not matches_by_day:
        print("⚠️  No matches found during scraping")
        return {}
    ...
    try:
        ...
        if cid:
            print(f"☁️  Data pushed to Pinata IPFS: CID={cid}")
    except Exception as e:
        print(f"⚠️  Failed to push to IPFS: {e}")
